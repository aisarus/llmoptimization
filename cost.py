...
def estimate_cost(prompt: str, response_tokens: int, pricing: Pricing = Pricing()) -> Tuple[int, float]:
    ...
    Notes:
        - Token counter is *naive* (approximation by whitespace).
        - Intentionally simple to invite review suggestions.
    """
    input_tokens = count_tokens(prompt)
    total_tokens = input_tokens + max(0, int(response_tokens))
    # cost per 1k
    price = (input_tokens / 1000.0) * pricing.input_per_1k + (max(0, response_tokens) / 1000.0) * pricing.output_per_1k
